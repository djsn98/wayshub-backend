...
def repo = "https://github.com/djsn98/wayshub-backend.git"
        ...
        stage('docker build'){
            steps{
                sshagent([cred]){
                    sh """ssh -o StrictHostKeyChecking=no ${builderserver} << EOF
                    cd ${directory}
                    docker build -t djsn98/wayshub-be:prod .
                    exit
                    EOF"""
                }
            }
	...
	stage('docker login'){
        ...
        stage('docker push'){
            steps{
                sshagent([cred]){
                    sh """ssh -o StrictHostKeyChecking=no ${builderserver} << EOF
                    docker push djsn98/wayshub-be:prod
                    exit
                    EOF"""
                }
            }
        ...
        stage('deploy'){
		    ...
		    docker image rm djsn98/wayshub-be:prod
